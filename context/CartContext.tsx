...
import type { CartAction, CartContextType, CartItem, CartState } from "@/utils/Types/cart";
    ...
    try {
      ...
      if (response.data.status && response.data.data?.data) {
        const items: CartItem[] = response.data.data.data.map((item: any) => ({
          ...
          image: item.product?.variant_item?.image || item.product?.images?.[0] || item.product?.image || "",
